#!/usr/bin/sh
#************************************************************************	
# Nombre      : reporteErrorActRenta.sh
# Descripcion : Proceso encargado de generar archivo con reporte errores 
#               en actualizacion de renta por atm para ser enviado via email.
# Autor	      : Andres Moran Ortiz
# Empresa     : SEnTRA
# Sintaxis    : sh reporteErrorActRenta.sh
# Fecha	      : 28-07-2016
#************************************************************************
# Mantencion : 
#************************************************************************
...
#---------------------------------------------------------------------
BaseDir="$HOME"
ENVFILE="${BaseDir}/cfg/profileReporteRentaError"
...
NombreInterfazError="TIMEOUT_ATM-"${fecPro}".CSV"
...
#--------------------------------------------------------
# Obtencion de clave del sistema COTIZA
#--------------------------------------------------------
...
if [ -f ${DIRDAT}/${ARCH_ULT_EJE_REP_ERR} ]; then
   fechaEjecucionAnterior=`cat ${DIRDAT}/${ARCH_ULT_EJE_REP_ERR}`
else
   fechaEjecucionAnterior=${fechaUltimaEjecucion}
fi

echo ${fechaUltimaEjecucion} > ${DIRDAT}/${ARCH_ULT_EJE_REP_ERR}

#Rescatamos los registros para la fecha 
printlog "- exec sp_cns_reg_act_tout ${fechaEjecucionAnterior}"
...
set nocount on
go
exec sp_cns_reg_act_tout "${fechaEjecucionAnterior}"
go
EOF
VerificaEjecucion "sp_cns_reg_act_tout"

cat ${OUTFILE} | tail +4 | awk 'length($0)>0'| grep -v "return status" | awk '{ print substr($0,2,length($0))}' | sed 's/.$//g' > ${DIRTMP}/${NombreInterfazError}

CANTREG=0
CANTREG=`wc -l ${DIRTMP}/${NombreInterfazError} | cut -f1 -d " " | awk '{printf ("%d", $0);}`

if [ $CANTREG -lt 1 ]; then
   printlog "- No se encontraron registros para el dia ${fechaEjecucionAnterior}"
   rm -f ${OUTFILE} 2>/dev/null
   rm -f ${DIRTMP}/${NombreInterfazError} 2>/dev/null
   SALIR 4
fi
...
echo "ath: ${NombreInterfazError}"     >> ${DIRTMP}/${archivoCorreo}
...
echo "put ${NombreInterfazError}"       >> $cmdFtp
echo "cd .." >> $cmdFtp
...
if [ $V_FTP -gt 0 ]; then 
   ...
   printlog "- Error al realizar el ftp del archivo ${NombreInterfazError}"
   printlog `tail  ${ftplog}`
   SALIR 2
fi

rm -f ${DIRTMP}/${NombreInterfazError}
